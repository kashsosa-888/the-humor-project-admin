...
export default async function DashboardPage() {
  ...
  const unratedCaptions = (totalCaptions ?? 0) - captionsWithVotes;
  const ratedPct =
    (totalCaptions ?? 0) > 0
      ? Math.round((captionsWithVotes / (totalCaptions ?? 1)) * 100)
      : 0;
  ...
  const voteCounts = [...voteMap.values()].map((v) => v.count);
  const distBuckets = [
    { label: "0 votes", count: unratedCaptions },
    { label: "1–2", count: voteCounts.filter((c) => c >= 1 && c <= 2).length },
    { label: "3–5", count: voteCounts.filter((c) => c >= 3 && c <= 5).length },
    { label: "6–10", count: voteCounts.filter((c) => c >= 6 && c <= 10).length },
    { label: "11+", count: voteCounts.filter((c) => c >= 11).length },
  ];
  const maxDistBucket = Math.max(...distBuckets.map((b) => b.count), 1);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          label="Avg Votes per Caption"
          value={avgVotesPerCaption}
          sublabel="across all captions"
        ...
        <StatCard
          ...
          value={`${ratedPct}%`}
          sublabel={`${captionsWithVotes} rated / ${unratedCaptions} unrated`}
          ...
          <div className="space-y-2">
            {distBuckets.map((b) => (
              <div key={b.label} className="flex items-center gap-3">
                <span className="w-12 text-right text-xs text-gray-500">{b.label}</span>
                <div className="h-2.5 flex-1 rounded-full bg-gray-800">
                  <div
                    className="h-2.5 rounded-full bg-cyan-600 transition-all"
                    style={{ width: `${(b.count / maxDistBucket) * 100}%` }}
                  />
                </div>
                <span className="w-8 text-xs text-gray-400">{b.count}</span>
              </div>
            ))}
          </div>
        </div>
